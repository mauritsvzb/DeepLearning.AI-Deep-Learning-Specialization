...
* Dev and test sets NEED to come from the SAME distribution, in order to stay accurate even when the target changes

<img src="https://github.com/mauritsvzb/DeepLearning.AI-Deep-Learning-Specialization/assets/13508894/4b57442e-ec25-4720-bac0-f15f07db0fd7.png" width="700" />
...
* The old way was valid for a number of examples ~ <10,000
In the modern deep learning era, if you have a million or more examples a reasonable split would be 98% training, 1% dev, 1% test; the dev/test set just needs to be big enough e.g., 10k and NN models nowadays require huge amounts of data to train on
...
* In the last example, if we choose the best algorithm by metric it would be "A", but if the users decide it will be "B": Thus in this case, we want and need to change our metric
* `OldMetric = (1/m) * sum(y_pred[i] != y[i] ,m)` where m is the number of Dev set items
* `NewMetric = (1/sum(w[i])) * sum(w[i] * (y_pred[i] != y[i]) ,m)` where `w[i] = 1` if x[i] is not porn and w[i] = 10 if x[i] is porn
...
<img src="https://github.com/mauritsvzb/DeepLearning.AI-Deep-Learning-Specialization/assets/13508894/8c0d7371-b995-43cb-98a8-6b0d05fe7537.png" width="700" />

* You won't surpass an error that's called "Bayes optimal error"
* There may be 2 reasons why accuracy rate slows down once human-level performance is exceeded:
  1. The range between human-level error and Bayes optimal error is small
  2. The techniques available to increase accuracy when ML performance is worse than human-level performance don't work well when human-level performance is exceeded
...
* Variance problem (= Dev error - Training error) when you want to reduce your dev set error to the levevel of the training error
...
* In some problems, deep learning has surpassed human-level performance, like:
  ...
  * Loan approval
* These examples are not natural perception tasks, but rather learn on structured data
* Humans are far better in natural perception tasks like computer vision and speech recognition
